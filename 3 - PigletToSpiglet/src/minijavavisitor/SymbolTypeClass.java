...
public class SymbolTypeClass extends SymbolType{
	...
	Map<String, SymbolTypePrimitive> upperDataMembersPrimitive = motherClass._dataMembersPrimitive;
	Map<String, SymbolTypeClass> upperDataMembersClass = motherClass._dataMembersClass;
	Map<String, List> upperMethods = motherClass.getMethods();

	_dataMembersPrimitive = new HashMap<String, SymbolTypePrimitive>();

	for( Map.Entry<String, SymbolTypePrimitive> pfield : upperDataMembersPrimitive.entrySet()){
	    _dataMembersPrimitive.put(pfield.getKey(), pfield.getValue());
	}

	_dataMembersClass = new HashMap<String, SymbolTypeClass>();
	for( Map.Entry<String, SymbolTypeClass> cfield : upperDataMembersClass.entrySet()){
	    _dataMembersClass.put(cfield.getKey(), cfield.getValue());
	}
	
	_methods = new HashMap<String, List>();
	for( Map.Entry<String, List> method : upperMethods.entrySet()){
	    _methods.put(method.getKey(), method.getValue());
	}
	...
	if (_methods.containsKey(name)){
	    if (isSubClass()&&(!_motherClass.hasMethod(name)) )
	...
	return _dataMembersClass.containsKey(field)||_dataMembersPrimitive.containsKey(field);
    }
